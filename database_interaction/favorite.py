...
def get_favorite_user_channels(user_id: int, limited: bool = False) -> [FavoriteChannel]:
...
def delete_favorite_user_channel(user_id: int, favorite_channel: FavoriteChannel):
...
WHERE user_id = %s AND channel_url = %s
            """, (user_id, favorite_channel.channel_id))
...
def get_favorite_user_videos(user_id: int, limited: bool = False) -> [FavoriteVideo]:
...
def delete_favorite_user_video(user_id: int, favorite_video: FavoriteVideo):
    """Удалить видео из списка избранного у пользователя"""
    with connection as connect:
        with connect.cursor() as curs:
            curs.execute("""
