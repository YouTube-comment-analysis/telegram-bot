...
def is_channel_url_correct(url):
    ...
    while not is_end:
        ...
        for comment_data in js['onResponseReceivedEndpoints'][index][continuation_name]['continuationItems']:
            if 'continuationItemRenderer' not in comment_data:
                comment = get_comment_from_comment_data(
                    comment_data['commentThreadRenderer']['comment']['commentRenderer'])
        ...
        for token in tokens:
            ...
            if 'continuationItems' in new_js['onResponseReceivedEndpoints'][0]['appendContinuationItemsAction']:
                ...
                while not is_sub_end:
                    is_sub_end = True
                    for comment_data in new_js['onResponseReceivedEndpoints'][0]['appendContinuationItemsAction'][
                        'continuationItems']:
                    ...
                    last_comment_data = \
                    new_js['onResponseReceivedEndpoints'][0]['appendContinuationItemsAction']['continuationItems'][-1]
                    if 'commentRenderer' not in last_comment_data:
                        sub_token = \
                        last_comment_data['continuationItemRenderer']['button']['buttonRenderer']['command'][
                            'continuationCommand']['token']
                        response = get_post_response('https://www.youtube.com/youtubei/v1/next', session, context,
                                                     sub_token, key)
                        new_js = response.json()
                        is_sub_end = False
        ...
        if 'continuationItemRenderer' in last_comment_data:
            token = last_comment_data['continuationItemRenderer']['continuationEndpoint']['continuationCommand'][
                'token']
            response = get_post_response('https://www.youtube.com/youtubei/v1/next', session, context, token, key)
            js = response.json()
            is_end = False
