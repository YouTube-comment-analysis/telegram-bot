...
class YoutubeCommentDownloader:
    ...
    def get_comments_from_url(self, youtube_url, sort_by=SORT_BY_RECENT, language=None, sleep=.1):
        ...
        while continuations:
            ...
            comment = Comment()
            for comment_data in reversed(list(self.search_dict(response, 'commentRenderer'))):
                comment.text = ''.join([c['text'] for c in comment_data['contentText'].get('runs', [])])
                comment.votes = comment_data.get('voteCount', {}).get('simpleText', '0')
                comment.is_reply = isreply
                result = {#'cid': comment['commentId'],
                          #'text': ''.join([c['text'] for c in comment_data['contentText'].get('runs', [])]),
                          #'time': comment['publishedTimeText']['runs'][0]['text'],
                          #'author': comment.get('authorText', {}).get('simpleText', ''),
                          #'channel': comment['authorEndpoint']['browseEndpoint'].get('browseId', ''),
                          #'votes': comment_data.get('voteCount', {}).get('simpleText', '0'),
                          #'isReply': isreply
                ...
                try:
                    comment.date = str(dateparser.parse(comment_data['publishedTimeText']['runs'][0]['text'].split('(')[0].strip()).date())
                    #result['date'] = str(dateparser.parse(comment['publishedTimeText']['runs'][0]['text'].split('(')[0].strip()).date())
